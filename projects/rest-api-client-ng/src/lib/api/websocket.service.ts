...
//import { szStreamConnProperties } from '../model/szStreamConnectionProperties';
import { WebSocketConnectionConfiguration, WebSocketConnectionParameters } from '../configuration';
...
export class SzPocWebSocketService {
  ...
  /*
  public connectionProperties: WebSocketConnectionParameters = {
  ...
  private webSocketConfiguration = new WebSocketConnectionConfiguration();

  public get connectionProperties(): WebSocketConnectionParameters {
    return this.webSocketConfiguration.connectionParameters
  }
  public set connectionProperties(value: WebSocketConnectionParameters) {
    this.webSocketConfiguration.connectionParameters = value;
  }
  public set path(value: string) {
    this.webSocketConfiguration.path = value;
  }
  public get path(): string {
    return this.webSocketConfiguration.path;
  }
  ...
  static getSocketUriFromConnectionObject(connProps: WebSocketConnectionParameters, path?: string, method?: "POST" | "PUT" | "GET"): string {
    ...
    if(!connProps) {
      // see if we can get it from the "webSocketConnection" configuration object
  ...
  /*
  constructor(protected httpClient: HttpClient, @Optional()@Inject(BASE_PATH) basePath: string, @Optional()@Inject(WEBSOCKET_BASE_PATH) wsPath: string, @Optional() configuration: Configuration) {
    if (basePath) {
    ...
    if(this.ws$) {
      let onWSExists = () => {
        ...
        this.ws$.pipe(
          ...
          this._reconnectionAttemptsIncrement = 0;
          console.log(`(${reconnected} | ${this._reconnectionAttemptsIncrement})!!successfully reconnected to "${WebSocketConnectionConfiguration.getSocketUriFromConnectionObject(this.connectionProperties)}"`, reconnected);
        })
      }
  ...
  public testConnection(connectionProps?: WebSocketConnectionParameters): Observable<boolean> {
    ...
    if(connectionProps) {
      let connProps = connectionProps ? connectionProps : this.connectionProperties;
      let _wsaddr = WebSocketConnectionConfiguration.getSocketUriFromConnectionObject(connProps, "/load-queue/bulk-data/records", "POST");
      ...
      console.info(`testing websocket connection ${_wsaddr}`, this.connectionProperties);
